...
namespace MedikTapp.Services.NavigationService
{
    ...
    public static class NavigationParametersExtensions
    {
        ...
        {
            foreach (var kvp in parameters.Where(kvp => string.Compare(kvp.Key, key, StringComparison.Ordinal) == 0))
            {
                if (TryGetValueInternal(kvp, type, out object value))
                    return value;
        ...
        public static bool TryGetValue<T>(this IEnumerable<KeyValuePair<string, object>> parameters, string key, out T value)
        {
            foreach (var kvp in parameters.Where(kvp => string.Compare(kvp.Key, key, StringComparison.Ordinal) == 0))
            {
                bool success = TryGetValueInternal(kvp, typeof(T), out object valueAsObject);
